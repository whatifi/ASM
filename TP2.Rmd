...
$lm(lcavol~1,data=pro)$ est une regression de taille 0.
$lm(lcavol~.,data=pro[,c(1,4,9)])$ est une regression linéaire de tous le prédicteurs par rapport aux variables 1,4 et 9.

$lm(lcavol~.,data=pro[,c(1,2,9)])$ est une regression linéaire de tous le prédicteurs par rapport aux variables 1,2 et 9.
...
RSSmin <- function(k){
  if( k == 0 ){
    ...
    for(i in 1:ncol(Combinaisons)){
      combinaison <- union(1,Combinaisons[,i])
...
plot(seq(0,8,1), Yplot, xlab = "Taille", ylab = "RSS minimale pou chaque k", type = "l")
...
```{r echo=FALSE, warning=FALSE}
#### selection des individues d'indice multiple de 3:
...
L'erreur d'apprentissage est donné par la cette commande $R$ :
 
```{r }
Ertraining <- sum(lmSplit$residuals**2)/32
```

#### c) L'ereur de prédiction:
...
```{r echo=FALSE, warning=FALSE}
#levels(proC$gleason) = levels(factor(proC[valid,7]))
#print(levels(proC$gleason))
LM =  lm(lcavol~.,data=pro[-valid,c(1,6,9)])
validation<-pro[valid, c(1,6,9)]
...
L'erreur de prediction est égale à $0.47931$ plus petit que l'erreur d'apprentissage $(0.9188223)$ ce qui est logique car le nombre d'individus pour l'apprentissage est plus petit. $(n1 = 32, n2 = 65)$.
...
Split_validation <- function(k){
  ...
  if (k==0){
    ...
    prediction <- predict(LM, pro[valid,])
    Ertraining <- sum(LM$residuals^2)/32
    Erprediction <- sum((pro[valid,"lcavol"]-prediction)^2)/65
  }  
  else{
    lmSplit <- lm(lcavol~.,data=proC[-valid,union("lcavol",RSSmin(k)$prednames)])
    Ertraining <- sum(lmSplit$residuals**2)/32
    LM =  lm(lcavol~.,data=proC[-valid,union("lcavol",RSSmin(k)$prednames)])
    validation<-proC[valid, union("lcavol",RSSmin(k)$prednames)]
    prediction<-predict(LM, newdata = validation )
    Erprediction <- sum((proC[valid,1]-prediction)**2)/65
  }
...
plot(seq(0,8,1), Yplot1, type = "n", ylim = range(c(Yplot1, Yplot2, Yplot3)), xlab = "k", ylab = "Erreurs ",main = "Valeurs de ErreurApprentissage(bleu) et ErreurPrédiction(rouge).")
lines(seq(0,8,1), Yplot1, col = "blue")
lines(seq(0,8,1), Yplot3, col = "pink")
lines(seq(0,8,1), Yplot2, col = "red")
```

On remarque pour l'erreur d'apprentissage le modèle optimale est celui de la taille 8, et pour l'erreur de Prédiction est celui de la taille 2 .En faisant la somme des erreurs on déduit que le modèle minimisant l'erreur global est celui de la taille 8.


#### Question e):
Le principal problème de la split-Validation est la multitude de choix pour choisir la valeur de n1 et n2 pour en déduire le modèle optimale. Du coup on peut tomber sur un modèle plus précis en modifiant n1 et n2.
